...
GH_MIRROR="${GH_MIRROR:-https://gh.llkk.cc/}"
...
retry() {
    ...
    until "$@"; do
        ...
        if [ $n -gt $MAX_RETRIES ]; then
            log "WARNING: $desc 重试 $MAX_RETRIES 次后仍然失败，跳过"
            return 1
        fi
...
if [ ! -d "/opt/sam2" ]; then
    log "克隆 SAM2 源码..."
    retry "SAM2源码" git clone --depth 1 https://github.com/facebookresearch/sam2.git /opt/sam2 || true
...
retry "SAM2权重" wget -q --show-progress -O "$MODEL_DIR/sam2/sam2.1_hiera_large.pt" "$SAM2_URL" || true

# ── 2. MediaPipe ──
log "安装 MediaPipe..."
retry "MediaPipe" pip install --no-deps --quiet mediapipe 2>/dev/null || pip install --quiet mediapipe || true

# ── 3. MotionBERT（GitHub Releases，走镜像）──
log "下载 MotionBERT..."
MOTIONBERT_DIRECT="https://github.com/Walter0807/MotionBERT/releases/download/v1.0.0/motionbert_ft_h36m.pth"
MOTIONBERT_MIRROR="${GH_MIRROR}https://github.com/Walter0807/MotionBERT/releases/download/v1.0.0/motionbert_ft_h36m.pth"
retry "MotionBERT" wget -q --show-progress -O "$MODEL_DIR/motionbert/motionbert_ft_h36m.pth" "$MOTIONBERT_MIRROR" 2>/dev/null || \
retry "MotionBERT直连" wget -q --show-progress -O "$MODEL_DIR/motionbert/motionbert_ft_h36m.pth" "$MOTIONBERT_DIRECT" || true
